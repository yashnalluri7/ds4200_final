...
function updateSankeyWithSelection() {
    ...
    d3.csv("data.csv").then(function(data) {
        ...
        createSankeyDiagram(data, selectedIndicators);
    }).catch(function(error) {
        console.error('Error loading or processing CSV file:', error);
    });
}

// Unified function name for all calls (renamed from SankeyDiagram)
function createSankeyDiagram(data, selectedIndicators) {
    ...
    svg.append("text")
        ...
        .text("Health Indicators Flow to Diabetes Status");
    ...
    try {
        ...
        data.forEach(d => {
            ...
            // Add links for BMI if selected - NOW FROM BMI TO DIABETES
            if (selectedIndicators.includes("BMI")) {
                let bmiNodeIndex;
                ...
                addLink(bmiNodeIndex, diabetesStatusIndex); // Flipped: BMI -> Diabetes
            }
            
            // Add links for HighBP if selected - NOW FROM HIGH BP TO DIABETES
            if (selectedIndicators.includes("HighBP")) {
                const highBPNodeIndex = d.HighBP === 0 ? 
                    indicatorNodeIndices.HighBP.no : 
                    indicatorNodeIndices.HighBP.yes;
                    
                addLink(highBPNodeIndex, diabetesStatusIndex); // Flipped: HighBP -> Diabetes
            }
            
            // Add links for HighChol if selected - NOW FROM HIGH CHOL TO DIABETES
            if (selectedIndicators.includes("HighChol")) {
                const highCholNodeIndex = d.HighChol === 0 ? 
                    indicatorNodeIndices.HighChol.no : 
                    indicatorNodeIndices.HighChol.yes;
                    
                addLink(highCholNodeIndex, diabetesStatusIndex); // Flipped: HighChol -> Diabetes
            }
            
            // Add links for GenHlth if selected - NOW FROM GEN HLTH TO DIABETES
            if (selectedIndicators.includes("GenHlth")) {
                const genHlthNodeIndex = indicatorNodeIndices.GenHlth[d.GenHlth];
                if (genHlthNodeIndex !== undefined) {
                    addLink(genHlthNodeIndex, diabetesStatusIndex); // Flipped: GenHlth -> Diabetes
                }
            }
            
            // Add links for Sex if selected - NOW FROM SEX TO DIABETES
            if (selectedIndicators.includes("Sex")) {
                const sexNodeIndex = indicatorNodeIndices.Sex[d.Sex];
                if (sexNodeIndex !== undefined) {
                    addLink(sexNodeIndex, diabetesStatusIndex); // Flipped: Sex -> Diabetes
                }
            }
            
            // Add links for Age if selected - NOW FROM AGE TO DIABETES
            if (selectedIndicators.includes("Age")) {
                let ageNodeIndex;
                ...
                addLink(ageNodeIndex, diabetesStatusIndex); // Flipped: Age -> Diabetes
            }
        });
        ...
        svg.append("g")
            ...
            .attr("d", sankeyLink)
            .attr("stroke", d => {
                // Color links based on target node (diabetes status) - Updated for flipped diagram
                if (d.target.index === 0) return "#3498db"; // No Diabetes
                if (d.target.index === 1) return "#f39c12"; // Prediabetic
                if (d.target.index === 2) return "#e74c3c"; // Diabetic
                return "#aaa";
            })
        ...
        const nodeColor = d => {
            // Color nodes based on their category - Updated for flipped diagram
            if (d.index <= 2) return "#3498db"; // Diabetes status nodes
            
            // Different colors for different indicator types
            if (selectedIndicators.includes("BMI") && 
                d.index >= indicatorNodeIndices.BMI.low && 
                d.index <= indicatorNodeIndices.BMI.high) {
                return "#e74c3c"; // BMI nodes
            }
            if (selectedIndicators.includes("HighBP") && 
                d.index >= indicatorNodeIndices.HighBP.no && 
                d.index <= indicatorNodeIndices.HighBP.yes) {
                return "#2ecc71"; // HighBP nodes
            }
            if (selectedIndicators.includes("HighChol") && 
                d.index >= indicatorNodeIndices.HighChol.no && 
                d.index <= indicatorNodeIndices.HighChol.yes) {
                return "#f39c12"; // HighChol nodes
            }
            if (selectedIndicators.includes("GenHlth") && 
                d.index >= indicatorNodeIndices.GenHlth[1] && 
                d.index <= indicatorNodeIndices.GenHlth[5]) {
                return "#9b59b6"; // GenHlth nodes
            }
            if (selectedIndicators.includes("Sex") && 
                d.index >= indicatorNodeIndices.Sex[0] && 
                d.index <= indicatorNodeIndices.Sex[1]) {
                return "#1abc9c"; // Sex nodes
            }
            if (selectedIndicators.includes("Age") && 
                d.index >= indicatorNodeIndices.Age.young && 
                d.index <= indicatorNodeIndices.Age.senior) {
                return "#34495e"; // Age nodes
            }
            
            return "#1abc9c"; // Default color
        };
...
function initVisualizations() {
    ...
    d3.csv("data.csv").then(function(data) {
        ...
        // Call the Sankey diagram function with initial indicators
        createSankeyDiagram(data, ["BMI", "HighBP", "HighChol"]);
